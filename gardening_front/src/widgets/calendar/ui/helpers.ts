...
import type {TCalendarView, TEventColor,} from "@/widgets/calendar/ui/types.ts";
import {ko} from "date-fns/locale/ko";

const FORMAT_STRING = "MMM d일";
...
export function rangeText(view: TCalendarView, date: Date): string {
    ...
    switch (view) {
        ...
        case "week":
            ...
            break;
        case "day":
            return format(date, FORMAT_STRING, {locale: ko});
        case "year":
    ...
    return `${format(start, FORMAT_STRING, {locale: ko})} - ${format(end, FORMAT_STRING, {locale: ko})}`;
}
...
export function calculateMonthEventPositions(
    ...
    sortedEvents.forEach((event) => {
        ...
        const eventDays = gridDates.filter((day) => {
            const d = startOfDay(day).getTime();
            return d >= eventStart.getTime() && d <= eventEnd.getTime();
        });
        ...
        for (let i = 0; i < 3; i++) {
            if (
                eventDays.every((day) => {
                    ...
                    return occupiedPositions[dayKey] && !occupiedPositions[dayKey][i];
                })
            ) {
                position = i;
                break;
...
export function formatTime(
    ...
    return format(parsedDate, use24HourFormat ? "HH:mm" : "a h:mm", {locale: ko});
}
