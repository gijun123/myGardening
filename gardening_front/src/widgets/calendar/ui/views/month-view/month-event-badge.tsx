...
import {DraggableEvent} from "@/widgets/calendar/ui/dnd/draggable-event.tsx";
            ...
            <EventDetailsDialog event={event}>
                <div role="button" tabIndex={0} className={eventBadgeClasses}>
                            ...
                            <p className="flex-1 truncate font-semibold">
                                {eventCurrentDay && (
                                    <span className="text-xs">
										총  {eventTotalDays}일 중 {eventCurrentDay}일째 •{" "}
									</span>
                                )}
                                {event.title}
                            </p>
                    ...
                    <div className="hidden sm:block">
                        {renderBadgeTime && (
                            <span>
                                {formatTime(new Date(event.startDate), use24HourFormat) + " "}시작
                            </span>
                        )}
                    </div>
                </div>
